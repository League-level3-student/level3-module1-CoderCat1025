	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == timer) {
			...
			for (int i = 0; i < cities.size(); i++) {
				...
				clocks = clocks + cities.get(i) + "\n" + dateStr + "\n" + timeStr + "\n";
			}
			textArea.setText(clocks);
			frame.pack();
		} else if (e.getSource() == button) {
			String cityName = JOptionPane.showInputDialog(null, "Input a city (city, country).");

			if (clockUtil.getTimeZoneFromCityName(cityName) != null) {
				cities.add(cityName);
				zones.add(clockUtil.getTimeZoneFromCityName(cityName));
			} 
			else {
				JOptionPane.showMessageDialog(null, "Couldn't find city.");
			}
		}
	}
}
