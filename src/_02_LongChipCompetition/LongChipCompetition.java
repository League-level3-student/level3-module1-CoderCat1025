...
public class LongChipCompetition {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < lcc.theBeatles.size(); i++) {
			...
			for(int e = 0; e < lcc.theBeatles.get(i).getChips().size(); e++) {

				if (e <= lcc.theBeatles.get(i).getChips().size() - 1) {
					
					if (lcc.theBeatles.get(i).getChips().get(i).getLength() > longest) {

						longest = lcc.theBeatles.get(i).getChips().get(i).getLength();
		...
		System.out.println("The Beatle with the longest chip is " + name + " whose chip was " + longest + " centimetres long.");
